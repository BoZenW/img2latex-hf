...
class ResNetTransformer(nn.Module):
    def __init__(
        ...
        #self.vit = VisionTransformer(self.d_model, 256, 3, nhead, 6, num_classes, 4, 128)
    ...
    def encode(self, x: Tensor) -> Tensor:
        ...
        encoding = self.vit_feature_extractor(images=x, return_tensors="pt") # convert to default size
        x = self.vit(**encoding).last_hidden_state # [B, H'*W', E]
        x = x.permute(1, 0, 2) # (Sx, B, E); Sx = H * W
        return x
